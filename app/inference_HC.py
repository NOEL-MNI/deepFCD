...
options['test_folder'] = '/host/silius/local_raid/ravnoor/01_Projects/06_DeepLesion/dataControls'
...
print('-'*80)
print("testing healthy controls")
print('-'*80)

tfolder = '/host/silius/local_raid/ravnoor/01_Projects/06_DeepLesion/dataControls'
test_list = os.listdir(tfolder)

modality = [x.upper() for x in modalities]
test_list = re.findall(r'\d{3}_\d{1}', ''.join(test_list)) # match 303_1, etc.
print("testing dataset size: {}".format(len(test_list)))
test_data = {f: {m: os.path.join(tfolder, f, m+'.nii.gz') for m in modality} for f in test_list}

for ix in enumerate(tqdm(test_list)):
    scan = ix[1]
